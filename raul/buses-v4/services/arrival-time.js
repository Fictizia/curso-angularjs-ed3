// Services
angular.module('app').service('arrivalTime', ['$http', '$firebaseArray', '$firebaseObject', arrivalTimeService]);

function arrivalTimeService($http, $firebaseArray, $firebaseObject) {
	...
	function fireGet() {
		...
		var ref = firebase.database().ref('list/-Ky5_YoU2O0UcusyI1-q');

		console.log(ref);

		arrivalTime.list = $firebaseArray(ref);

	}

	function fireSave(newValue) {
		console.log('saving...');
		arrivalTime.object['$value'] = newValue;
		arrivalTime.object.$save();
	}

	function fireAdd(newValue) {
		console.log('adding...');
		//arrivalTime.object['$value'] = newValue;
		arrivalTime.list.$add({message: newValue});
	}
